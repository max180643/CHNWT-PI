...
sudo docker run -d -p 139:139 -p 445:445 --name samba --restart=always -e USERID=1000 -e GROUPID=1000 -e TZ=Asia/Bangkok -v ~/myTorrent:/mount1 -v ~/myData:/mount2 dperson/samba:latest -p -u "${USER};${PASWORD}" -s "chnwtPI-Torrent;/mount1;no;no;no;max180643" -s "chnwtPI-Data;/mount2;no;no;no;max180643" || error "Failed to run Samba docker image!"